...
import 'fake_saved_meal_source.dart';
...
void main() {
  group('FakeSavedMealSource', () {
    ...
    test('never produces PermissionDenied', () async {
      final source = FakeSavedMealSource();
      final meal = _meal(id: 'm1', name: 'Chicken salad');
      ...
      for (final result in [saveResult, listResult, deleteResult]) {
        if (result case Err(failure: final failure)) {
          expect(failure, isNot(isA<PermissionDenied>()));
        }
      }
    });
  });
